...
function Wargames(app, options) {
	...
	self.settings = {
		...
		ircPassword: options.ircPassword || null,
		ircPort: options.ircPort || 6667,
		ircSSL: options.ircSSL || false,
		ircDebug: options.ircDebug || false,
...
Wargames.prototype.initiateIrcBot = function () {
	...
	var bot = new irc.Client(self.settings.ircNetwork, self.settings.ircBotNick, {
		...
		'password': self.settings.ircPassword,
		'port': self.settings.ircPort,
		'secure': self.settings.ircSSL,
		'debug': self.settings.ircDebug,
		'showErrors': self.settings.ircDebug
	});
